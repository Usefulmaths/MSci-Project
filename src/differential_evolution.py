...
def write_to_file(file_name, iters, minimum, agent, time):
    ...
    file_open.write(str(iters) + ", " + str(minimum) + ", " +
                    str(time) + ", " + str(agent) + "\n")
    file_open.close()
...
def adaptive_differential_evolution(func, iterations, number_of_agents,
                                    number_of_parameters, CR, F, bounds,
                                    hard_bounds, file_name,
                                    threshold_value=10e100, thread=False):
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    size = comm.Get_size()
    minimum = 10E10
    ...
    if(rank == 0):
        agents = generate_agents(
            number_of_agents, number_of_parameters, bounds)
    ...
    while(True):
        start = time()
        data = agent_procedure(
            func, agents, number_of_parameters, hard_bounds, F, CR, rank)
        comm.Barrier()
        end = time()
        ...
        if(iters % 50 == 0):
            ...
            if(rank == 0):
                minimum, optimised_agent = min(
                    evaluated_agents, key=lambda t: t[0])
                print(minimum, optimised_agent)
                write_to_file(file_name, iters, minimum,
                              optimised_agent, end - start)
